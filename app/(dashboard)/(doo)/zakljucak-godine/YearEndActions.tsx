...
export default function YearEndActions({ year, accountingProfit, alreadyClosed }: Props) {
  ...
  const inv = parseFloat(investmentDeduction) || 0;

  const taxableProfit = Math.max(0, accountingProfit + nd - nti - lc - inv);
  const corporateTax = Math.round(taxableProfit * TAX_RATE * 100) / 100;
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Korak 2 — Porezni bilans */}
      <div className="rounded-lg border p-5 space-y-4">
        <h3 className="font-semibold">Korak 2 — Porezni bilans (PB-800-A) · stopa 10%</h3>
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
          ...
          <div className="flex flex-col gap-1">
            <Label className="text-xs text-muted-foreground">- Investiciona olakšica (50% ulaganja u OS)</Label>
            <Input type="number" min={0} step="0.01" value={investmentDeduction} onChange={e => setInvestmentDeduction(e.target.value)} className="h-8 font-mono text-sm" />
          </div>
          ...
          <div className="col-span-2 border-t pt-2 space-y-1.5">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Porez na dobit (10%)</span>
      ...
      <div className="rounded-lg border p-5 space-y-3">
        <div>
          <h3 className="font-semibold">Korak 3 — Zaključak poslovne godine</h3>
      ...
      <div className="rounded-lg border p-5 space-y-3">
        <div>
          <h3 className="font-semibold">Korak 4 — Otvaranje {year + 1}. godine</h3>
          <p className="text-sm text-muted-foreground mt-0.5">
