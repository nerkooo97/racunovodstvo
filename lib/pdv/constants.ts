/**
 * Konstante za e-KIF / e-KUF prema TehniÄkom uputstvu UIO BiH.
 * Tipovi dokumenata: bazno uputstvo 01â05, izmjene 09/2023 proÅ¡iruju na 01â09.
 */
...
import { getCurrentTaxConfig } from "@/lib/constants/tax-config";

/**
 * Standardna stopa PDV-a u BiH — u PROCENTIMA (17), za PDV forme koje rade
 * na skali 0-100. IZVOR ISTINE je lib/constants/tax-config.ts (skala 0-1);
 * ovdje se samo preračunava. Kad se PDV stopa promijeni, mijenja se SAMO
 * tax-config.ts — ovaj izraz automatski prati.
 */
export const VAT_RATE = getCurrentTaxConfig().vatRate * 100;

/** Tipovi dokumenta KUF (nabavke) */
export const KUF_DOCUMENT_TYPES = [
  { code: "01", label: "Ulazna faktura (roba/usluge iz zemlje)" },
  { code: "02", label: "Vlastita potroÅ¡nja (vanposlovne svrhe)" },
  { code: "03", label: "Avansna faktura (dati avans)" },
  { code: "04", label: "JCI (uvoz)" },
  { code: "05", label: "Usluge primljene iz inostranstva" },
  { code: "06", label: "KnjiÅ¾no odobrenje (KO)" },
  { code: "07", label: "Ostali dokument (poslovne knjige)" },
  { code: "08", label: "Ulaz â posebno" },
...
export const KIF_DOCUMENT_TYPES = [
  ...
  { code: "05", label: "Usluge izvrÅ¡ene stranom licu / ostalo osloboÄeno" },
  { code: "06", label: "KnjiÅ¾no odobrenje (KO)" },
  { code: "07", label: "Ostali dokument (poslovne knjige)" },
  { code: "08", label: "Izlaz â posebno" },
  { code: "09", label: "Donacija hrane / posrednik" },
] as const;

export type UioDocumentType = string;

/** Tipovi dokumenata dostupni u aplikaciji (01â09 po izmjenama 09/2023). */
...
export const NON_DEDUCTIBLE_REASONS: {
  ...
  { value: "representation", label: "Reprezentacija (ruÄkovi, pokloni)" },
  { value: "vehicle", label: "PutniÄko vozilo (nabavka/odrÅ¾avanje)" },
  { value: "mixed", label: "MjeÅ¡ovita upotreba (srazmjerni odbitak)" },
...
export const SALE_CATEGORIES: { value: SaleCategory; label: string }[] = [
  ...
  { value: "domestic_b2c", label: "Prodaja neobvezniku / graÄaninu" },
  { value: "export_goods", label: "Izvoz robe (JCI)" },
  { value: "export_services", label: "Usluge stranom licu" },
  { value: "exempt", label: "OsloboÄeno bez prava na odbitak" },
  { value: "internal_use", label: "Vlastita / vanposlovna potroÅ¡nja" },
];

/** CSV tehniÄka pravila (UIO) */
